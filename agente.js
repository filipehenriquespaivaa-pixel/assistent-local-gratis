...
const VERBOS_CONSTRUCAO = /\b(crie|criar|cria|construa|construir|desenvolva|desenvolver|fa[çc]a|fazer|monte|montar|programe|programar|quero|preciso|gostaria|implemente|implementar)\b/i;
const VERBOS_EDICAO = /\b(adicione|adicionar|mude|mudar|altere|alterar|corrija|corrigir|conserte|consertar|arrume|arrumar|melhore|melhorar|ajuste|ajustar|remova|remover|atualize|atualizar|refatore|refatorar|continue|continuar|troque|trocar)\b/i;
...
const SUBSTANTIVOS_CONSTRUCAO = /\b(jogo|game|app|aplicativo|site|p[aá]gina|landing\s?page|sistema|script|programa|calculadora|to-?do|lista de tarefas|api|bot|chatbot|automa[çc][ãa]o|ferramenta|dashboard|painel|planilha|formul[aá]rio|extens[ãa]o|plugin|componente|endpoint|rota|relat[oó]rio|macro)\b/i;
...
async function planejar(conversationHistory, systemPrompt, mensagemUsuario, arquivosMencionados) {
  ...
  const instrucaoPlano = {
    ...
    content:
      ...
      'LÓGICA:\n' +
      '- descreva em poucas linhas a lógica principal (loop do jogo, colisão, controles, regras, fluxo de dados, etc.)\n\n' +
